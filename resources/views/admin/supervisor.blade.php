      ...
      <div class="card my-5">
        <div class="card-header">
                    <h4>Gerenciar Supervisores
                        <button type="button" class="btn btn-md btn-primary float-right" role="button" data-toggle="modal" data-target="#newModalsupervisor" data-toggle="tooltip" data-placement="left" title="Clique para abrir o formulário de novo defensor"><i class="fa fa-plus"></i> Novo Defensor</button>
                    </h4>
        </div>
        <div class="card-body">
          <div class="col-lg-12">
                  ...
                  <table class="table table-striped">
                    ...
                    <tbody>
                      @forelse($supervisors as $supervisor)
                        @if($supervisor->user->type == "supervisor" && $supervisor->status == "active")
                          <tr class="object align-middle" name="{{$supervisor->name}}">
                            ...
                            <td style="font-size:18pt;width:15%" class="text-center">
                              <button type="button" class="btn btn-outline-warning" role="button" data-toggle="modal" data-target="#editModalsupervisor" onclick="editsupervisor('{{$supervisor->id}}','{{$supervisor->name}}','{{$supervisor->user->email}}','{{$supervisor->gender}}','{{$supervisor->phone}}')"><i class="fa fa-edit"></i></button>
                              <button type="button" class="btn btn-outline-danger" role="button" data-toggle="modal" data-target="#deleteModalsupervisor" onclick="deletesupervisor('{{$supervisor->id}}','{{$supervisor->name}}')"><i class="fa fa-trash"></i></button>
                            </td>
                          </tr>
                        @endif
                      @empty
...
<!-- Modal -->
<div class="modal fade" id="newModalsupervisor" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
...
<!-- Modal -->
<div class="modal fade" id="deleteModalsupervisor" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-body">
        <form action="{{URL::to('Admin/Defensor/Excluir')}}" method="post">
          {{ csrf_field() }}
          <input type="hidden" name="id" id="deleteIdsupervisor" value="">
          ...
          <h4 class="text-center"><strong id="deleteNamesupervisor"></strong></h4>
          <br>
          <div class="text-center">
